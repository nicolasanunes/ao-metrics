...
<template>
  <div class="bg-gray-950/90 text-gray-100 rounded-lg p-6">
      ...
      <div class="bg-gray-900 rounded-xl p-4 flex-1 flex flex-col gap-4">
        <div>
          ...
          <div class="flex flex-wrap gap-1.5 text-xs">
            ...
            >
              {{ isSeedSustainable ? '✓ Sustentável' : '✗ Não sustentável' }}
            </span>
          </div>
        </div>
    ...
    <div class="bg-gray-900 rounded-xl p-6">
      ...
      <div v-if="plots > 1" class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-red-900/20 border border-red-700/40 rounded-xl p-4 text-center">
          <p class="text-xs text-gray-400 mb-1">Custo total ({{ plots }} fazendas)</p>
        ...
        >
          <p class="text-xs text-gray-400 mb-1">Lucro total ({{ plots }} fazendas)</p>
      ...
      <div class="overflow-x-auto mb-6">
        <table class="w-full text-sm border-collapse">
          ...
          <tbody>
            ...
            <!-- Earthworms row -->
            <tr class="border-t border-gray-800 hover:bg-gray-800/40 transition-colors">
              <td class="px-3 py-2 text-yellow-300">Minhoca</td>
      ...
      <div class="border-t border-gray-700 pt-5">
        <h3 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">
          Lucratividade (por fazenda)
        </h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="bg-gray-800 rounded-xl p-3 text-center">
            <p class="text-xs text-gray-400 mb-1">Lucro bruto</p>
            <p class="text-xl font-bold text-gray-200">{{ fmt(revenue) }}</p>
          </div>
          <div class="bg-gray-800 rounded-xl p-3 text-center">
            <p class="text-xs text-gray-400 mb-1">Custo de sementes</p>
            <p class="text-xl font-bold text-red-400">{{ fmt(cost) }}</p>
          </div>
          <div class="bg-gray-800 rounded-xl p-3 text-center">
            <p class="text-xs text-gray-400 mb-1">Lucro líquido</p>
            <p class="text-xl font-bold" :class="profitColorClass(netProfit)">
              {{ fmt(netProfit) }}
            </p>
          </div>
